    ...
    public void build() {
        ...
        try {
            ...
            for (int i = 0; i < (SortType.values().length); i++) {
                ...
                //verify Jar
                ProcessBuilder processBuilder = new ProcessBuilder(Configuration.INSTANCE.pathToJarsignerDirectory, "-verify", Configuration.INSTANCE.pathToGenericComponentDirectory + SortType.values()[i] + Configuration.INSTANCE.sorterComponentFolderName + Configuration.INSTANCE.pathToGenericComponentJavaArchive + SortType.values()[i] + Configuration.INSTANCE.sorterJarName);
